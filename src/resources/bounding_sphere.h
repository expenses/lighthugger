...
static void computeBoundingSphere(float result[4], const float* points, size_t count)
{
	assert(count > 0);
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		{
			pmin[axis] = (p[axis] < points[3 * pmin[axis] + axis]) ? i : pmin[axis];
			pmax[axis] = (p[axis] > points[3 * pmax[axis] + axis]) ? i : pmax[axis];
		}
	}
