...
MeshInfo load_mesh_info(uint64_t address) {
    ...
    info.bounding_sphere = load_value_and_increment_address<float4>(address);
...
Meshlet load_meshlet(uint64_t address, uint32_t offset) {
    ...
    meshlet.bounding_sphere = load_value_and_increment_address<float4>(address);
...
uint16_t4 load_uint8_t3(uint64_t address) {
    return unpack_u8u16(vk::RawBufferLoad<uint8_t4_packed>(address, 1));
}
