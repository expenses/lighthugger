...
void read_depth() {
    ...
    uint4 depth_1 = asuint(textureGather(sampler2D(depth_buffer, clamp_sampler), (coord + uint2(1, 1)) * pixel_size));
    uint4 depth_2 = asuint(textureGather(sampler2D(depth_buffer, clamp_sampler), (coord + uint2(1, 3)) * pixel_size));
    uint4 depth_3 = asuint(textureGather(sampler2D(depth_buffer, clamp_sampler), (coord + uint2(3, 1)) * pixel_size));
    uint4 depth_4 = asuint(textureGather(sampler2D(depth_buffer, clamp_sampler), (coord + uint2(3, 3)) * pixel_size));
