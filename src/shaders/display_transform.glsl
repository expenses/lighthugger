...
float3 tony_mc_mapface(float3 stimulus) {
    ...
    return textureLod(sampler3D(display_transform_lut, clamp_sampler), uv, 0.0)
        .xyz;
}
...
float3 linear_to_srgb_transfer_function(float3 value) {
    return float3(
        linear_to_srgb_transfer_function(value.x),
        linear_to_srgb_transfer_function(value.y),
        linear_to_srgb_transfer_function(value.z)
    );
}
...
void display_transform() {
    ...
    float3 srgb_encoded_value =
        linear_to_srgb_transfer_function(linear_display_referred_value);
