...
for file in $shaders; do
    dxc -HV 2021 $error_flags -spirv -fspv-target-env=vulkan1.1 -T lib_6_7 -enable-16bit-types src/shaders/$file -Fo compiled_shaders/$(basename $file .hlsl).spv || exit 1
done
